...
| Path | Contents |
|---|---|
| `README.md` | Business question, findings, choices, and limitations |
| `notebooks/gate_experiment_analysis.ipynb` | Complete executed analysis |
| `figures/` | Three figures generated by the notebook |
| `outputs/results.json` | Estimates, assumptions, data fingerprint, and package versions |
| `outputs/*.csv` | Retention, rounds, and bootstrap summaries |
| `requirements.txt` | Pinned direct dependencies |
| `.gitignore` | Local data, environments, and notebook checkpoints excluded |
| `data/` | Data instructions; CSV downloaded separately |

### Data source

Download [**gate_experiment.csv from Google Drive**](https://drive.google.com/file/d/1CVMsQtjLeW-pf_LN0cm5zlaWTn1tjeyF/view?usp=sharing) and save it as `data/gate_experiment.csv`. The CSV is not committed to the repository or included in the project ZIP.

### Running

Locally, extract the project folder and run:
...
In Colab, open the notebook and choose **Runtime → Run all**. No Drive connection is required. Data is read from `data/gate_experiment.csv` or downloaded from the public source link if absent. Figures and summaries are written to the project folder printed by the setup cell.

### Optional: save results directly to your own Drive

Upload the extracted `Mobile_game_retention_ABtest/` folder to My Drive and open its notebook with Colab. Use Colab's **Files** panel to connect Drive before running. The default folder is detected automatically; if stored elsewhere, set `PROJECT_PATH` to the mounted project path. The notebook does not request Drive access itself.

After running, save the notebook, wait for saving to finish, and download the project folder from Drive. The same folder can be uploaded to GitHub. If working on a separate notebook copy, replace the project notebook with the executed copy first. See `COLAB_GUIDE.md` for the short workflow.
